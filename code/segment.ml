...
let arg_spec_list =["--badScore", Arg.Set_string badScore, " Score assigned when word length is less than window size (Default = 0.0)";
                    ...
                    "--nBest", Arg.Set_int nBestWindow, " Store scores for N-best segmentations instead of just the best. Should help lessen impact of early errors.";
                    "-nb", Arg.Set_int nBestWindow, " Short for --nBest";
...
(* Returns a list of pairs of strings and StringSets that represents all of the precedence relationships in the given word. *)
let get_piecewise_set_list word =
    let indexArray = Array.init (String.length word) (fun a -> a) in
    fst (Array.fold_left
            (fun (currentList, currentSeenSet) currentFirstIndex ->
                let currentPhoneme = (String.sub word currentFirstIndex 1) in
                let newSet = (StringSet.add currentPhoneme currentSeenSet) in
                if (not (StringSet.is_empty currentSeenSet)) then
                    ((currentList @ [(currentPhoneme, currentSeenSet)]), newSet)
                else
                    (currentList, newSet)
            )
            ([], StringSet.empty)
...
struct
    ...
    let rec discount n wordTypesWithCountArray =
        match n with
            0 -> wordTypesWithCountArray.(0) / (wordTypesWithCountArray.(0) + (2 * wordTypesWithCountArray.(1)))
        |   1 | 2 -> n - ((n + 1) * (discount 0 wordTypesWithCountArray) * (wordTypesWithCountArray.(n) / (wordTypesWithCountArray.(n - 1))))
        | _ -> 3 - (4 * (discount 0 wordTypesWithCountArray) * (wordTypesWithCountArray.(4) / (wordTypesWithCountArray.(3))));;
...
struct
    ...
    (* Calculates probability of a phoneme given a set of of phonemes that came before it. *)
    let prob_piecewise_given_set (phoneme:string) set =
        let prob_piecewise_helper (currentPhoneme:string) = StringSet.fold
                                                                (fun prefix currentProduct ->
                                                                    currentProduct */ (prob_ngram prefix (prefix ^ currentPhoneme) 1 ngramCountsArray totalNgramsArray [||] ngramCountsArray initialCountsArray)
                                                                )
                                                                set
                                                                (num_of_int 1)
        in
        let numerator = prob_piecewise_helper phoneme in
        let denominator = List.fold_left
                            (fun currentSum currentPhoneme ->
                                (prob_piecewise_helper currentPhoneme) +/ currentSum
                            )
                            (num_of_int 0)
                            !phonemeList
        in
    ...
    let eval_word (word:string) combine =
        ...
        if (String.length wordWithBoundary) < !phonemeWindow then
            ...
            begin
                score := (num_of_int 1);
                List.iter (* Get piecewise scores *)
                    (fun (currentPhoneme, currentSet) ->
                        let piecewiseScore = prob_piecewise_given_set currentPhoneme currentSet in
                        score := (combine !score piecewiseScore)
                    )
                    (get_piecewise_set_list wordWithBoundary);
                !score
            end
...
struct
    ...
    let compare (score1, firstChar1) (score2, firstChar2) =
        BatNum.compare score1 score2
end;;
...
(* Finds the path through bestStartList which reveals the starts and stops of all hypothesized words in the utterance.
         sure you call this with firstChar set to the length of the utterance*)
let rec find_segmentation_pairlist bestStartList (firstChar:int) path =
    if firstChar > 0 then
        begin
            let (_, newFirstChar) = PairSet.max_elt (bestStartList.(firstChar - 1)) in
            find_segmentation_pairlist bestStartList newFirstChar (path @ [newFirstChar])
        end
    else
        path;;

let rec nbest_inner subUtterance firstChar lastChar bestList n =
    if firstChar <= lastChar then
        begin
            let newSubUtterance = String.sub subUtterance firstChar ((lastChar + 1) - firstChar) in
            update_subseq_count newSubUtterance (num_of_int 1);
            ...
            if scoreProduct >/ lastCharBestProduct then
                ...
                nbest_inner subUtterance (firstChar + 1) lastChar (Array.concat [(Array.sub bestList 0 lastChar); [| lastCharPairSet |]; (Array.sub bestList (lastChar + 1) ((Array.length bestList) - (lastChar + 1)))]) n
            else
                nbest_inner subUtterance (firstChar + 1) lastChar bestList n
        end
    else
        bestList;;

(* Keeps track of the N-best segmentations instead of just the best *)
let nbest_outer n sentence =
    let lastCharList = Array.init (String.length sentence) (fun a -> a) in
    let bestList = Array.fold_left
        (fun oldBestList lastChar ->
            ...
            let subUtterance = String.sub sentence 0 (lastChar + 1) in
            update_subseq_count subUtterance (num_of_int 1);
            let newBestList = Array.append oldBestList [| PairSet.add ((eval_word subUtterance), 0) PairSet.empty |] in
            nbest_inner subUtterance 1 lastChar newBestList n
        )
        [||]
        lastCharList
    in
    let sentLength = Array.length bestList in
    PairSet.fold
        (fun (score, lastWordStart) scoredSegList ->
            let segList = (find_segmentation_pairlist bestList lastWordStart []) @ [sentLength] in
            Array.append [|(score, segList)|] scoredSegList
        )
        bestList.(sentLength - 1)
        [||];;

let rec viterbi_inner subUtterance firstChar lastChar bestList =
    if firstChar <= lastChar then
        begin
            let newSubUtterance = String.sub subUtterance firstChar ((lastChar + 1) - firstChar) in
            update_subseq_count newSubUtterance (num_of_int 1);
            ...
            if scoreProduct >/ lastCharBestProduct then
                ...
                viterbi_inner subUtterance (firstChar + 1) lastChar bestList
        end
    else
        bestList;;

let viterbi_outer sentence =
    let lastCharList = Array.init (String.length sentence) (fun a -> a) in
    let bestList = Array.fold_left
        (fun oldBestList lastChar ->
            (* eprintf "LastChar: %i\tString length: %i\n" lastChar (String.length sentence); *)
            ...
            viterbi_inner subUtterance 1 lastChar newBestList
        )
        [||]
        lastCharList
    in
...
let eval_utterance = viterbi_outer;;
...
(* let get_scored_segmentation_list = nbest_outer !nBestWindow;; *)
