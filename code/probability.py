...
#         Daniel Blanchard <dan.blanchard@gmail.com> (additions)
#
# URL: <http://www.nltk.org/>
# For license information, see LICENSE.TXT
#
# $Id: probability.py 8776 2011-04-10 06:43:28Z StevenBird1 $
...
class FreqDist(dict):
    ...
    """
    def __init__(self, samples=None, counttype=int):
        """
        ...
        @param counttype: The data type to use to store the counts.
        @type counttype: C{numbers.Number}
        """
        dict.__init__(self)
        self._type = counttype
        self._N = self._type(0)
        self._reset_caches()
        if samples:
            self.update(samples)

    def inc(self, sample, count=None):
        """
    ...
    # unsure why this used to return a list of floats
    ...
    def freq(self, sample):
        ...
        @rtype: float, if self._type = int, and self._type otherwise.
    ...
    def plot(self, *args, **kwargs):
        ...
        pylab.xticks(range(len(samples)), [str(float(s)) for s in samples], rotation=90)
    ...
    def tabulate(self, *args, **kwargs):
        ...
        for i in range(len(samples)):
            print "%4s" % str(float(samples[i])),
        print
        for i in range(len(samples)):
            print "%4d" % float(freqs[i]),
        print
...
#            items = [(sample, self[sample]) for sample in set(samples)]
    ...
    def update(self, samples, inc_amount=1):
        ...
        @type samples: C{list}
        @param inc_amount: Amount to increment the count for each sample by.
        @type inc_amount: C{numbers.Number}
        """
        try:
            sample_iter = samples.iteritems()
        except:
            sample_iter = imap(lambda x: (x,self._type(inc_amount)), samples)
...
class ProbDistI(object):
    ...
    def update(self, samples, inc_amount=1):
        """
        Update the underlying frequency distribution to add the given samples.
        Also recomputes any values that would be calculated the ProbDist's initializer
        that would need to be updated.

        @param samples: The samples that should have their frequencies updated.
        @type samples: C{list}
        @param inc_amount: The amount to increment the frequency of the samples by.
        @type inc_amount: C{numbers.Number}
        """
...
class UniformProbDist(ProbDistI):
    ...
    samples.
    """
    def __init__(self, samples, probtype=float):
        """
        ...
        @param probtype: The data type to be used to store the probabilities.
        @type probtype: C{numbers.Number} (but not C{int})
        @raise ValueError: If C{probtype} is C{int}.
        """
        if len(samples) == 0:
        ...
        if probtype is int:
            raise ValueError('Cannot store probabilities as ints.')
        self._probtype = probtype
        self._prob = probtype(1)/len(self._sampleset)
    ...
    def update(self, samples, inc_amount=1):
        ...
        self._sampleset.update(samples)
        self._samples.extend(samples)
        self._prob = self._probtype(1)/len(self._sampleset)
...
class DictionaryProbDist(ProbDistI):
    ...
    def update(self, prob_dict):
        """
        Update the current probability distribution to include any new probabilities
        in C{prob_dict}, replacing any old conflicting values.
        @param prob_dict: Dictionary that maps from values to new probabilities.
        @type prob_dict: C{dict}
        """
        self._prob_dict.update(prob_dict)
...
class HeldoutProbDist(ProbDistI):
    ...
    def __init__(self, base_fdist, heldout_fdist, bins=None):
        ...
        # Type to use for frequency calculations (use at least float)
        self._type = float if base_fdist._type is int else base_fdist._type
...
class CrossValidationProbDist(ProbDistI):
    ...
    def __init__(self, freqdists, bins):
        ...
        # Type to use for frequency calculations (use at least float)
        if (not freqdists) or freqdists[0]._type is int:
            self._type = float
        else:
            self._type = freqdists[0]._type
...
class GoodTuringProbDist(ProbDistI):
    ...
    def prob(self, sample):
        ...
        if count == 0 and self._freqdist.N() != 0:
            p0 = self._type(1) * self._freqdist.Nr(1) / self._freqdist.N()
            if self._bins == self._freqdist.B():
                p0 = self._type(0)
            else:
                p0 = p0 / (self._type(1) * self._bins - self._freqdist.B())
        ...
        return self._type(1) * (count + 1) * ncn / (nc * self._freqdist.N())
    ...
    def discount(self):
        ...
        @rtype: C{float}
        """
        return self._type(1) * self._freqdist.Nr(1) / self._freqdist.N()
...
class SimpleGoodTuringProbDist(ProbDistI):
    ...
    def find_best_fit(self, r, nr):
        ...
        for j in range(len(r)):
            ...
            if j != len(r) - 1:
                ...
                k = 2 * r[j] - i
            zr_ = self._type(2) * nr[j] / (k - i)
            zr.append(zr_)

        log_r = [math.log(i) for i in r]
        log_zr = [math.log(i) for i in zr]

        xy_cov = x_var = self._type(0)
        x_mean = self._type(1) * sum(log_r) / len(log_r)
        y_mean = self._type(1) * sum(log_zr) / len(log_zr)
    ...
    def _switch(self, r, nr):
        ...
        for i, r_ in enumerate(r):
            ...
            unsmooth_r_star = self._type(1) * (r_ + 1) * nr[i+1] / nr[i]
    ...
    def _variance(self, r, nr, nr_1):
        ...
        nr = float(nr)
        nr_1 = float(nr_1)
        return (r + self._type(1))**2 * (nr_1 / nr**2) * (self._type(1) + nr_1 / nr)
    ...
    def prob(self, sample):
        ...
        if count == 0:
            if self._bins == self._freqdist.B():
                ...
                p = p / (self._type(1) * self._bins - self._freqdist.B())
        else:
            p = p * self._renormal
        return p
    ...
    def _prob_measure(self, count):
        ...
        if self._switch_at > count:
            Er_1 = self._type(1) * self._freqdist.Nr(count+1)
            Er = self._type(1) * self._freqdist.Nr(count)
        else:
    ...
    def check(self):
        prob_sum = self._type(0)
        for i in  range(0, len(self._Nr)):
            prob_sum += self._Nr[i] * self._prob_measure(i) / self._renormal
        print "Probability Sum:", prob_sum
        #assert prob_sum != self._type(1), "probability sum should be one!"
    ...
    def discount(self):
        ...
        seen samples to the unseen samples.
        """
        return  self._type(1) * self.smoothedNr(1) / self._freqdist.N()
...
class ConditionalFreqDist(object):
    ...
    """
    def __init__(self, cond_samples=None, counttype=int):
        """
        ...
        @param counttype: The data type to use to store the counts in
            the underlying C{FreqDist}s.
        @type counttype: C{numbers.Number}
        """
        self._fdists = {}
        self._type = counttype
    ...
    def __getitem__(self, condition):
        ...
        if condition not in self._fdists:
            self._fdists[condition] = FreqDist(counttype=self._type)
...
class ConditionalProbDist(ConditionalProbDistI):
    ...
    distribution for each condition is an C{ELEProbDist} with 10 bins:
        ...
        0.0813
    """
    def __init__(self, cfdist, probdist_factory, counttype=int,
    ...
    def __getitem__(self, condition):
        if condition not in self._pdists:
            ...
            pdist = self._probdist_factory(FreqDist(counttype=self._type),
                                           *self._factory_args,
